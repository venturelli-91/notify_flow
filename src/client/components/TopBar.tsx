...
function ChevronDownIcon() {
	return (
		<svg className="h-3.5 w-3.5 text-gray-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
		...
		<header className="flex items-center gap-4 mb-5">
			...
			<div className="flex-1 flex items-center bg-white rounded-xl shadow-sm pl-4 pr-2 py-2 gap-3">
				...
				<button
					type="button"
					aria-label="Search"
					className="h-8 w-8 rounded-full bg-teal-700 flex items-center justify-center hover:bg-teal-800 transition-colors flex-shrink-0"
				>
					<SearchIcon />
				</button>
			</div>
			...
			>
				...
				<span className="absolute -top-0.5 -right-0.5 h-3 w-3 rounded-full bg-red-500 border-2 border-[#dde8f0]" />
			</button>
			...
			<div className="relative flex-shrink-0" ref={ref}>
				...
				>
					{/* Avatar circular */}
					<div className="h-9 w-9 rounded-full bg-teal-700 flex items-center justify-center text-white font-semibold text-sm ring-2 ring-white shadow-sm">
						A
					</div>
					<ChevronDownIcon />
				</button>
				...
				{open && (
					<div className="absolute right-0 top-[calc(100%+10px)] w-56 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
						...
						<div className="flex items-center gap-3 px-4 py-4 border-b border-gray-100">
							<div className="h-10 w-10 rounded-full bg-teal-700 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
								A
							</div>
							<div className="min-w-0">
								<p className="text-sm font-semibold text-gray-900 truncate">Admin</p>
								<p className="text-xs text-gray-400 truncate">admin@notifyflow.com</p>
							</div>
						</div>
